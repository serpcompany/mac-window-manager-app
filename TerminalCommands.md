...
```bash
defaults write com.serp.windowmanager alternateDefaultShortcuts -bool true
```
...
```bash
defaults write com.serp.windowmanager subsequentExecutionMode -int 2
```
...
2: disabled
3: cycle displays for left/right actions, halves to thirds for the rest (old Window Manager behavior)
...
```bash
defaults write com.serp.windowmanager centerHalfCycles -int 1
```
...
```bash
defaults write com.serp.windowmanager snapModifiers -int 1048576
```
...
```bash
defaults write com.serp.windowmanager almostMaximizeHeight -float <VALUE_BETWEEN_0_&_1>
```

```bash
defaults write com.serp.windowmanager almostMaximizeWidth -float <VALUE_BETWEEN_0_&_1>
```
...
```bash
defaults write com.serp.windowmanager specified -dict-add keyCode -float 8 modifierFlags -float 1966080
```

```bash
defaults write com.serp.windowmanager specifiedHeight -float 1050
defaults write com.serp.windowmanager specifiedWidth -float 1680
```
...
```bash
defaults write com.serp.windowmanager topLeftThird -dict-add keyCode -float 18 modifierFlags -float 917504
```
...
```bash
defaults write com.serp.windowmanager doubleWidthRight -dict-add keyCode -float 124 modifierFlags -float 11403555
```
...
```bash
defaults write com.serp.windowmanager cascadeActiveApp -dict-add keyCode -float 2 modifierFlags -float 393475
```
...
```bash
defaults write com.serp.windowmanager footprintAlpha -float <VALUE_BETWEEN_0_&_1>
```

Change the border width. Default is 2 (used to be 1).

```bash
defaults write com.serp.windowmanager footprintBorderWidth -float <NUM_PIXELS>
```

Disable the fade.

```bash
defaults write com.serp.windowmanager footprintFade -int 2
```

Change the color.

```bash
defaults write com.serp.windowmanager footprintColor -string "{\"red\":0,\"blue\":0.5,\"green\":0.5}"
```

Change the animation duration. The value is a multiplier. Default is 0 (no animation).

```bash
defaults write com.serp.windowmanager footprintAnimationDurationMultiplier -float <MULTIPLIER>
```
...
```bash
defaults write com.serp.windowmanager centeredDirectionalMove -int 2
```
...
Set either value to `0` to disable Window Manager's screen-relative limit for that dimension and rely on the application's native minimum size.

```bash
defaults write com.serp.windowmanager minimumWindowWidth -float <VALUE_BETWEEN_0_&_1>
```

```bash
defaults write com.serp.windowmanager minimumWindowHeight -float <VALUE_BETWEEN_0_&_1>
```
...
```bash
defaults write com.serp.windowmanager sizeOffset -float <NUM_PIXELS>
```
...
```bash
defaults write com.serp.windowmanager largerHeight -dict-add keyCode -float 30 modifierFlags -float 917504
defaults write com.serp.windowmanager smallerHeight -dict-add keyCode -float 33 modifierFlags -float 917504
```
...
```bash
defaults write com.serp.windowmanager smallerShrinksMaximizedHeight -bool true
```

## Disabling window restore when moving windows

```bash
defaults write com.serp.windowmanager unsnapRestore -int 2
```
...
```bash
defaults write com.serp.windowmanager snapEdgeMarginTop -int 10
defaults write com.serp.windowmanager snapEdgeMarginBottom -int 10
defaults write com.serp.windowmanager snapEdgeMarginLeft -int 10
defaults write com.serp.windowmanager snapEdgeMarginRight -int 10
```
...
```bash
defaults write com.serp.windowmanager screenEdgeGapTop -int 10
defaults write com.serp.windowmanager screenEdgeGapBottom -int 10
defaults write com.serp.windowmanager screenEdgeGapLeft -int 10
defaults write com.serp.windowmanager screenEdgeGapRight -int 10
```
...
```bash
defaults write com.serp.windowmanager screenEdgeGapTopNotch -int 5
```
...
```bash
defaults write com.serp.windowmanager screenEdgeGapsOnMainScreenOnly -bool true
```
...
```bash
defaults write com.serp.windowmanager ignoredSnapAreas -int 1
```

To disable the Top Half and Bottom Half snap areas, the bit field would be 1111 0000 0000, or 3840

```bash
defaults write com.serp.windowmanager ignoredSnapAreas -int 3840
```
...
```bash
defaults write com.serp.windowmanager applyGapsToMaximize -int 2
```

To disable the gaps for "Maximize Height", execute:

```bash
defaults write com.serp.windowmanager applyGapsToMaximizeHeight -int 2
```
...
```bash
defaults write com.serp.windowmanager sixthsSnapArea -bool true
```
...
```bash
defaults write com.serp.windowmanager moveCursor -int 1
```

## Control Enhanced UI handling

Some apps enable the macOS `AXEnhancedUserInterface` accessibility mode. Window Manager disables this mode while moving or resizing a window because its animated window updates can otherwise produce incorrect frames. Chromium browsers can also enable expensive web accessibility processing when the mode is restored.

Window Manager uses automatic handling by default: it restores Enhanced UI for other apps, but leaves it disabled after Window Manager window actions, when known Chromium browser families activate, and when one is already frontmost as Window Manager starts. VoiceOver and Switch Control keep the prior restore behavior. The behavior can be overridden with:

```bash
defaults write com.serp.windowmanager enhancedUI -int <MODE>
```
...
| Mode | Behavior |
|------|----------|
| `1` | Always restore Enhanced UI after a Window Manager window action if it was enabled beforehand. Use this when assistive software depends on the mode. |
...
```bash
defaults write com.serp.windowmanager missionControlDragging -int 2
```

Change the allowed off-screen distance. The value is in pixels and is tied to the speed. Default is 25.

```bash
defaults write com.serp.windowmanager missionControlDraggingAllowedOffscreenDistance -float <DISTANCE>
```

Change the disallowed duration. The value is in milliseconds. Default is 250.

```bash
defaults write com.serp.windowmanager missionControlDraggingDisallowedDuration -int <DURATION>
```

## Change the behavior of double-click window title bar

To change the action ([list](https://github.com/rxhanson/Rectangle/blob/master/Window Manager/WindowAction.swift)):

```bash
defaults write com.serp.windowmanager doubleClickTitleBar -int <ACTION_ID + 1>
```

To disable restore when double-clicked again:

```bash
defaults write com.serp.windowmanager doubleClickTitleBarRestore -int 2
```

To disable double-click window title bar only for specific bundle ids (in example, Outlook):

```bash
defaults write com.serp.windowmanager doubleClickTitleBarIgnoredApps -string "[\"com.microsoft.Outlook\"]"
```
...
```bash
defaults write com.serp.windowmanager screensOrderedByX -int 1
```
...
```bash
defaults write com.serp.windowmanager autoMaximize -int 2
```

To restore the default behavior:

```bash
defaults write com.serp.windowmanager autoMaximize -int 0
```

## Attempt to preserve window position when moving to another display

By default, moving a window to the next, previous, or a specific display centers it on the destination display. Enable this to instead try preserving the window's position on the destination. If the previous action was a Window Manager snap (half, third, maximize, etc.), that snap is replayed on the destination display. If the window was positioned manually, its rect is mapped proportionally from the source display to the destination display (a window at the right third stays at the right third) and clamped so it never overflows. This is off by default.

```bash
defaults write com.serp.windowmanager attemptMatchOnNextPrevDisplay -int 1
```
...
```bash
defaults write com.serp.windowmanager moveFixedSizeToEdge -int 1  # align edges and corners (default)
defaults write com.serp.windowmanager moveFixedSizeToEdge -int 2  # align corners only, center halves/sides
defaults write com.serp.windowmanager moveFixedSizeToEdge -int 3  # center within the snap area
```
