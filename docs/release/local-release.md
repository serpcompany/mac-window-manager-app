...
Wait until Apple reports the version as `READY_FOR_DISTRIBUTION` (legacy accounts may report `READY_FOR_SALE`). Then run the GitHub release harness once without `--confirm` and inspect its target:

```bash
scripts/publish_github_release.sh "$VERSION" "$RELEASE_COMMIT"
```

Publish after the preflight names the intended version, commit, tag, and App Store state:
...
The harness creates and pushes an annotated `v<version>` tag, then publishes a matching GitHub Release in `serpcompany/mac-window-manager-app`. It is resumable when the correct tag or release already exists. It never attaches the exported `.pkg`: Mac App Store packages are receipt-bound submission artifacts, not direct-download installers.
...
- published, non-draft GitHub Release for that tag
