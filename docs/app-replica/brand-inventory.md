...
| --- | --- | --- | --- | --- | --- |
| product.name | Name | Rectangle | replace | Window Manager (working name; final owner decision) | Search sources and built strings |
| product.bundle | Identifier | com.knollsoft.Rectangle | replace | com.serp.windowmanager | Inspect built Info.plist and defaults domain |
| launcher.bundle | Identifier | com.knollsoft.RectangleLauncher | replace | com.serp.windowmanager.launcher | Inspect built helper Info.plist |
| tests.bundle | Identifier | dev.ryanhanson.RectangleTests | replace | com.serp.windowmanager.tests | Inspect build settings |
| url.scheme | Integration | rectangle | replace | windowmanager | Launch URL action and inspect Info.plist |
...
| defaults.domain | Persistence | com.knollsoft.Rectangle | replace | com.serp.windowmanager | Fresh launch and plist audit |
| status.autosave | Persistence | Generic status-item visibility record | replace | `com.serp.windowmanager.statusItem` | Relaunch with menu icon enabled and inspect candidate defaults |
